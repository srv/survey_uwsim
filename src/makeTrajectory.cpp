...
void vehiclePoseCallback(const nav_msgs::Odometry& odom) {
  if (!firstpass) {
    initialT.set(odom.pose.pose.position.x,
                 odom.pose.pose.position.y,
                 odom.pose.pose.position.z);
    initialQ.set(odom.pose.pose.orientation.x,
    ...
    currentT.set(odom.pose.pose.position.x,
                 odom.pose.pose.position.y,
                 odom.pose.pose.position.z);
...
int main(int argc, char **argv) {
  ...
  double x,y,z,roll,pitch,yaw,longX,longY;
  int nx,ny;
  std::string poseTopic,controlTopic;
  ...
  nhp.getParam("y",y);
  nhp.getParam("z",z);
  nhp.getParam("roll",roll);
  nhp.getParam("pitch",pitch);
  nhp.getParam("yaw",yaw);
  nhp.getParam("longX",longX);
  nhp.getParam("longY",longY);
  nhp.getParam("nx",nx);
  nhp.getParam("ny",ny);
  nhp.param<std::string>("poseTopic",poseTopic,"pose");
  nhp.param<std::string>("controlTopic",controlTopic,"control");
  ...
  while (ros::ok()) {
    ...
    for (int i = 0; i < trajectory.size(); i++) {
      ...
      osg::Quat quatAux; // /Used as an auxiliar variable for getting the
                         // vehicle orientation

      while (ros::ok() &&
             ((!firstpass) || (currentDistance >= 0.05) ||
              (fabs(currentAngleDiff) >= 0.05))) {
        if (firstpass) {
          ...
          std::cout << "VScale=" << vScale << " vT.length()=" << vT.length() <<
          ...
          double yawT =
            angles::shortest_angular_distance(currentYaw, goalYaw) * 0.07;
          double yawScale = (fabs(yawT) > 0.1) ? fabs(YAWRATE / yawT) : 1.0;

          // std::cout << " currentYaw="<< currentYaw << " goalYaw=" << goalYaw
          // << " currentAngleDiff=" << currentAngleDiff << " totalAngleDiff="
          // << totalAngleDiff << std::endl;
          // std::cout << " yawT=" <<  yawT << "" << std::endl;
          // currentQ.slerp(1-currentDistance/totalDistance,initialQ, goalQ);

          /*if (fabs(totalAngleDiff)>=001)
             currentQ.slerp(1.0-fabs(currentAngleDiff/totalAngleDiff),initialQ,
                goalQ);
             else
          ...
          // Rz.makeRotate(currentYaw + yawT *
          // yawScale,0,0,1)*Ry*Rx*T).getRotate();
          ...
          if (totalDistance > 0.05) {
            odom.pose.pose.position.x = currentT.x() + vScale *
                                        (goalT.x() -
                                         initialT.x()) / totalDistance;
            odom.pose.pose.position.y = currentT.y() + vScale *
                                        (goalT.y() -
                                         initialT.y()) / totalDistance;
            odom.pose.pose.position.z = currentT.z() + vScale *
                                        (goalT.z() -
                                         initialT.z()) / totalDistance;
          } else {
