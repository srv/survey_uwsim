...
int SurveyMaker::update(void){
  ...
  tf::Vector3 vT = waypoint_position-current_position;

  double vScale = (vT.length()>linear_speed_) ? 0.15*linear_speed_/vT.length() : 0.15;  

  double yaw  = tf::getYaw(current_rotation);
  double waypoint_yaw  = tf::getYaw(waypoint_rotation);
  double diff_yaw = current_rotation.angleShortestPath(waypoint_rotation); 
  
  int rotation_dir;

  if(yaw > 0)
  {
    if(yaw - waypoint_yaw >= 0)
      rotation_dir = -1;
    else if(yaw - waypoint_yaw < 0)
      rotation_dir = 1;
  }
  else if(yaw < 0)
  {
  ...
  double yawT = diff_yaw * 0.2;//0.07
  double yawScale = (fabs(yawT) > 0.1) ? fabs(0.05 / yawT) : 0.5;

  double x = current_position.x() + vT.x()*vScale;
  double y = current_position.y() + vT.y()*vScale;
  double z = current_position.z() + vT.z()*vScale;

  ROS_INFO_STREAM("speeds in x = " << vT.x()*vScale << " in y = " << vT.y()*vScale << " in z = " << vT.z()*vScale);

  yaw += rotation_dir * fabs(yawT) * yawScale;
